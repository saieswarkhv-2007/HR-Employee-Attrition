...
# SOFTMAX REGRESSION FOR MULTI-CLASS HR EMPLOYEE ATTRITION PREDICTION
#
# IMPORTANT:
...
DATASET_FILE = "/Users/khvsaieswar/Desktop/HR_Employee_Attrition/dataset/final_preprocess.csv"
TARGET_COLUMN = "Department"
OUTPUT_FOLDER = "/Users/khvsaieswar/Desktop/HR_Employee_Attrition/outputs/Multinomial_Logistic_Regre_for_Multiclass"
...
print("SOFTMAX REGRESSION - MULTI-CLASS HR ATTRITION PREDICTION")
print("=" * 75)
...
with open(information_file, "w", encoding="utf-8") as file:
    file.write("PREPROCESSED HR ATTRITION DATASET INFORMATION\n")
    file.write("=" * 75 + "\n\n")
...
with open(metrics_file, "w", encoding="utf-8") as file:
    ...
    file.write("-" * 40 + "\n")
    for number, class_name in enumerate(class_names):
        file.write(f"{number} = {class_name}\n")
    file.write("\nMODEL PERFORMANCE\n")
    file.write("-" * 40 + "\n")
...
for i, class_name in enumerate(class_names):
    probability_column = "Probability_" + str(class_name).replace(" ", "_")
    prediction_output[probability_column] = y_probability[:, i]
...
probability_output = pd.DataFrame(y_probability)
probability_output.columns = [
    "Probability_" + str(class_name).replace(" ", "_")
...
plt.title("HR Department Class Distribution")
...
coefficients = pd.DataFrame(
    model.coef_,
    columns=X.columns,
    index=[str(x) for x in class_names]
)
coefficients.to_csv(
    os.path.join(
        OUTPUT_FOLDER,
...
intercepts = pd.DataFrame({
    "Class": [str(x) for x in class_names],
