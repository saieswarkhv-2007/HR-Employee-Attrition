# ============================================================
# HR EMPLOYEE ATTRITION - REGULARISATION
...
FILE_NAME = "/Users/khvsaieswar/Desktop/HR_Employee_Attrition/dataset/final_preprocess.csv"
output_folder = "/Users/khvsaieswar/Desktop/HR_Employee_Attrition/outputs/Regularization_ridg_laso_elastic_outputs"
...
print("\n================================================")
print("ORIGINAL PREPROCESSED HR ATTRITION DATASET")
print("================================================")
...
comparison.to_csv(
    os.path.join(output_folder, "regularisation_model_comparison.csv"),
    index=False
)

print(
    "\nSaved:\n"
    "regularisation_model_comparison.csv"
)
...
coefficient_table.to_csv(
    os.path.join(output_folder, "regularisation_coefficients.csv"),
    index=False
)

print(
    "\nSaved:\n"
    "regularisation_coefficients.csv"
)
...
if len(zero_lasso) > 0:
    ...
    print("You can increase alpha if stronger L1 regularisation is required.")
...
lasso_features["Selected_by_Lasso"] = (
    np.abs(lasso_features["Lasso_L1"]) > 1e-6
)

lasso_features.to_csv(
    os.path.join(output_folder, "lasso_feature_selection.csv"),
    index=False
)

print(
    "\nSaved:\n"
    "lasso_feature_selection.csv"
)
...
# L1 constraint: |b1| + |b2| = C
C = 1.5
L1 = np.abs(BETA1) + np.abs(BETA2)

# L2 constraint: b1^2 + b2^2 = C^2
L2 = BETA1**2 + BETA2**2

plt.figure(figsize=(8, 8))

# L1 diamond
plt.contour(BETA1, BETA2, L1, levels=[C], linewidths=2)

# L2 circle
plt.contour(BETA1, BETA2, L2, levels=[C**2], linewidths=2)
...
image_path = os.path.join(output_folder, "L1_vs_L2_Regularisation.png")
...
plt.bar(x - width, ridge.coef_, width, label="Ridge (L2)")
plt.bar(x, lasso.coef_, width, label="Lasso (L1)")
...
plt.bar(x - width, ridge_scores, width, label="Ridge")
plt.bar(x, lasso_scores, width, label="Lasso")
plt.bar(x + width, elastic_scores, width, label="Elastic Net")

plt.xticks(x, metrics)
plt.ylim(0, 1.1)
plt.ylabel("Score")
plt.title("Regularisation Model Performance")
plt.legend()
plt.grid(axis="y")
plt.tight_layout()
...
print("\n================================================")
print("VERIFY ORIGINAL HR ATTRITION PREPROCESSED DATASET")
print("================================================")
