# ============================================================
# HR EMPLOYEE ATTRITION - PREPROCESSED DATASET
...
OUTPUT_FOLDER = os.path.join(
    BASE_DIR,
    "outputs",
    "K_Means_K++Means_Elbow_Silhoute_Outputs"
)
...
print("HR EMPLOYEE ATTRITION - PREPROCESSED DATASET")
print("=" * 75)
...
for feature in required_features:
    ...
    if normalized_feature in normalized_columns:
        ...
        # Check numeric fallback if feature not directly matched
        numeric_candidates = data.select_dtypes(include=np.number).columns.tolist()
        if len(numeric_candidates) >= 4 and len(feature_columns) < 4:
            for cand in numeric_candidates:
                if cand not in feature_columns and cand.lower() != "attrition":
                    feature_columns.append(cand)
...
ax.set_title(
    "K-Means Clustering - HR Employee Attrition"
)
...
def cluster_matching_accuracy(
    ...
    return accuracy_score(
        true_values,
        predicted_values.astype(true_values.dtype)
    )
...
if target_column is not None:
    print("\n" + "=" * 75)
    print("ATTRITION CLUSTER MATCHING ACCURACY")
    print("=" * 75)
    ...
    accuracy_result.to_csv(
        os.path.join(
            ACCURACY_FOLDER,
            "attrition_cluster_matching_accuracy.csv"
        ),
        index=False
    )
else:
    print(
        "\nNo Attrition/Status/Target column was found."
    )
